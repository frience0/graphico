...
// Define the date range type to match the frontend
interface DateRange {
  from: Date
  to: Date
}
...
export async function getAllOrders({
  ...
  const effectiveLimit = limit ?? pageSize // Use pageSize as fallback if limit is undefined
  await connectToDatabase()
  const skipAmount = (Number(page) - 1) * effectiveLimit
  ...
  return {
    ...
    totalPages: Math.ceil(ordersCount / effectiveLimit),
  }
}
...
export async function getOrderSummary(date: DateRange) {
  ...
  const limit = pageSize // Define limit explicitly here
